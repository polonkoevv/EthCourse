...
func (p *Postgres) GetMusicById(ctx context.Context, id int) (*model.Music, error) {
	...
	err := p.conn.QueryRow(ctx, "SELECT * FROM music WHERE music_id = $1", id).Scan(&music.ID, &music.Title, &music.CID)
	if err != nil {
		return nil, err
	}
	return &music, nil
...
func (p *Postgres) GetMusicByCID(ctx context.Context, cid string) (*model.Music, error) {
	var music model.Music
	err := p.conn.QueryRow(ctx, "SELECT * FROM music WHERE cid = $1", cid).Scan(&music.ID, &music.Title, &music.CID)
	if err != nil {
		return nil, err
	}
	return &music, nil
...
func (p *Postgres) GetAllMusic(ctx context.Context) ([]model.Music, error) {
	...
	for rows.Next() {
		var m model.Music
		err := rows.Scan(&m.ID, &m.Title, &m.CID)
		if err != nil {
			return nil, err
		}
...
func (p *Postgres) CreateMusic(ctx context.Context, music model.Music) (int, error) {
	var id int
	err := p.conn.QueryRow(ctx, "INSERT INTO music (title, cid) VALUES ($1, $2) RETURNING music_id", music.Title, music.CID).Scan(&id)
	if err != nil {
		return 0, err
	}
	return id, nil
...
func (p *Postgres) UpdateMusic(ctx context.Context, music model.Music) error {
	_, err := p.conn.Exec(ctx, "UPDATE music SET title = $1, cid = $2 WHERE music_id = $3", music.Title, music.CID, music.ID)
	if err != nil {
		return err
	}
	return nil
